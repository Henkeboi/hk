...
bool file::open(std::string filename)
{
    try {
        ...
        while (std::getline(file, line)) {
            _buffer.get_buffer().push_back(std::make_tuple(std::wstring{}, true));
            int x_curser = 0;
            for (int i = 0; i < line.size(); ++i) {
                if (x_curser == _max_line_size) {
                    std::get<1>(_buffer.get_buffer().back()) = false;
                    _buffer.get_buffer().push_back(std::make_tuple(std::wstring{}, true));
                    x_curser = 0;
                }
                std::wstring s;
                ...
                std::get<0>(_buffer.get_buffer().back()) += wide_char; 
                ++x_curser;
            }
        }

        // init _buffer.get_buffer() if file is empty
...
bool file::save(std::string filename)
{
    auto string_to_write = [this]() {
        ...
        for (auto line_info : _buffer.get_buffer()) {
...
{ 
    if (x_curser - _left_margin >= get_line(line_number).size()) std::get<0>(_buffer.get_buffer().at(line_number)) += symbols;
    else std::get<0>(_buffer.get_buffer().at(line_number)).insert(x_curser - _left_margin, symbols);
    if (get_line(line_number).size() > _max_line_size) {
        if (is_end_line(line_number) == true) {
            std::get<1>(_buffer.get_buffer().at(line_number)) = false;
            auto next_line_iter = _buffer.get_buffer().begin() + line_number + 1;
            _buffer.get_buffer().insert(next_line_iter, std::make_tuple(std::wstring{}, true));
        }
        auto next_line = std::get<0>(_buffer.get_buffer().at(line_number)).substr(_max_line_size);
        std::get<0>(_buffer.get_buffer().at(line_number)) = std::get<0>(_buffer.get_buffer().at(line_number)).substr(0, _max_line_size);
...
void file::insert_symbol(int x_curser, int line_number, wint_t symbol)
{
    ...
    auto simple_insert_symbol = [this](int x_curser, int line_number, wint_t symbol) {
        ...
        s += c;
        wchar_t wide_char[] = {s.at(0), 0x00};
        std::get<0>(_buffer.get_buffer().at(line_number)).insert(x_curser - _left_margin, wide_char);
    };

    if (is_end_line(line_number) == true) {
        simple_insert_symbol(x_curser, line_number, symbol);
        if (std::get<0>(_buffer.get_buffer().at(line_number)).size() > _max_line_size) {
            auto next_line_symbol = std::get<0>(_buffer.get_buffer().at(line_number)).back();
            auto next_line_tuple = std::make_tuple(std::wstring{next_line_symbol}, true);
            auto next_line_iter = _buffer.get_buffer().begin() + line_number + 1;
            _buffer.get_buffer().insert(next_line_iter, next_line_tuple);
            std::get<1>(_buffer.get_buffer().at(line_number)) = false;
            std::get<0>(_buffer.get_buffer().at(line_number)).pop_back();
        }
    } else {
        simple_insert_symbol(x_curser, line_number, symbol);
        auto next_line_symbol = std::get<0>(_buffer.get_buffer().at(line_number)).back(); 
...
{
    assert(line_number < _buffer.get_buffer().size());
    auto current_line = std::get<0>(_buffer.get_buffer().at(line_number));
    std::wstring next_line = L"";
    auto next_line_iter = _buffer.get_buffer().begin() + line_number + 1;
    next_line += current_line.substr(x_curser - _left_margin, current_line.size());
    _buffer.get_buffer().insert(next_line_iter, std::make_tuple(next_line, true));
    auto prev_line = current_line.substr(0, x_curser - _left_margin);
    _buffer.get_buffer().at(line_number) = std::make_tuple(prev_line, true);
}
...
void file::delete_symbol(int x_curser, int line_number)
{
    ...
    if (x_curser - _left_margin == 0) {
        ...
        if (!(get_line(line_number - 1).size() == _max_line_size)) {
            if (is_end_line(line_number) && (line_number != _buffer.get_buffer().size() - 1)) {
                if (get_line(line_number + 1).size() + get_line(line_number).size() <= _max_line_size) {
                    std::get<1>(_buffer.get_buffer().at(line_number - 1)) = true;
                }
            } 
            auto line = std::get<0>(_buffer.get_buffer().at(line_number));
            _buffer.get_buffer().erase(_buffer.get_buffer().begin() + line_number);
            insert_symbols(get_line(line_number - 1).size() + _left_margin, line_number - 1, line);
        }
    } else {
        auto erase_iter = std::get<0>(_buffer.get_buffer().at(line_number)).begin() + x_curser - _left_margin - 1;
        std::get<0>(_buffer.get_buffer().at(line_number)).erase(erase_iter);
        if (is_end_line(line_number) == false) { // append the first letter of next line
            auto next_line_symbol = std::get<0>(_buffer.get_buffer().at(line_number + 1)).front(); 
            std::get<0>(_buffer.get_buffer().at(line_number)) += next_line_symbol;
            delete_symbol(_left_margin + 1, line_number + 1);
            if (get_line(line_number + 1).size() == 0) {
                _buffer.get_buffer().erase(_buffer.get_buffer().begin() + line_number + 1);
                std::get<1>(_buffer.get_buffer().at(line_number)) = true;
            }
        }
    }
}
...
{
    if (y_end - y_start > 1) {
        auto erase_start = _buffer.get_buffer().begin() + y_start + 1;
        auto erase_end = _buffer.get_buffer().begin() + y_end;
        _buffer.get_buffer().erase(erase_start, erase_end);
    }
    ...
    if (y_start == y_end) {
        if (x_start == 0 && x_end == std::get<0>(_buffer.get_buffer().at(y_start)).size()) {
            auto erase_start = _buffer.get_buffer().begin() + y_start;
            _buffer.get_buffer().erase(erase_start);
   
        } else delete_substring(x_start, x_end, y_start);
    } else {
        delete_substring(x_start, std::get<0>(_buffer.get_buffer().at(y_start)).size(), y_start);
        auto erase_start = _buffer.get_buffer().begin() + y_start + 1;
        _buffer.get_buffer().erase(erase_start);
    }
}
...
{
    assert(line_number <= _buffer.get_buffer().size());
    return std::get<1>(_buffer.get_buffer().at(line_number));
}

int file::get_number_of_lines()
{
    return _buffer.get_buffer().size();
}
...
void file::resize()
{
    ...
    lines.reserve(_buffer.get_buffer().size());
    int x_index = 0;
    for (auto line : _buffer.get_buffer()) { 
